...
import { useConversations, useMessages, useChatSocket, useGetOrCreateConversation } from '@/features/chat/hooks/useChat';
...
export default function InboxPage() {
  ...
  useEffect(() => {
    ...
    } else if (conversations && conversations.length > 0 && !activeConversationId && !initialListingId) {
  ...
  const otherUser = activeConversation?.renterId === user?.id ? activeConversation?.owner : activeConversation?.renter;
        ...
        <div className="white-card w-full flex overflow-hidden h-[calc(100vh-8rem)]">
          ...
          <div className="w-full md:w-1/3 border-r border-neutral-200 flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto">
              {isLoadingConvos ? (
                <div className="flex justify-center p-8"><Loader2 className="h-6 w-6 animate-spin text-primary-500" /></div>
                ...
                conversations?.map((conv) => {
                  ...
                  return (
                    ...
                    >
                      <img
                        src={participant?.avatarUrl || `https://api.dicebear.com/7.x/initials/svg?seed=${participant?.name || 'U'}`}
                        alt={participant?.name || 'User'}
                        className="h-12 w-12 rounded-full object-cover bg-white"
                      />
                      <div className="flex-1 min-w-0">
                        <div className="flex justify-between items-baseline">
                          <h3 className="font-semibold text-neutral-900 truncate">{participant?.name || 'Unknown'}</h3>
                          {latestMsg && (
                        ...
                        <p className="text-sm text-primary-600 truncate mt-0.5">{conv.listing?.title}</p>
            ...
            {activeConversationId ? (
              <>
                ...
                <div className="p-4 border-b border-neutral-200 flex items-center gap-3 bg-white">
                  <img
                        src={otherUser?.avatarUrl || `https://api.dicebear.com/7.x/initials/svg?seed=${otherUser?.name || 'U'}`}
                        alt={otherUser?.name || 'User'}
                        className="h-10 w-10 rounded-full object-cover"
                  />
                  <div>
                    <h3 className="font-semibold text-neutral-900">{otherUser?.name}</h3>
                    <p className="text-xs text-neutral-500">Regarding: {activeConversation?.listing?.title}</p>
                  </div>
                </div>
                    ...
                    [...(messagesData?.data || [])].reverse().map((msg) => {
                      ...
                      return (
                        <div key={msg.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                          ...
                          >
                            <p>{msg.content}</p>
                            <span className={`text-[10px] mt-1 block ${isMe ? 'text-primary-100' : 'text-neutral-400'}`}>
