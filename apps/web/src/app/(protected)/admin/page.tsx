...
export default function AdminDashboardPage() {
  ...
  const { data: usersData, isLoading: isLoadingUsers } = useAdminUsers(
    activeTab === 'users' ? page : 1,
  );
  const { data: listingsData, isLoading: isLoadingListings } = useAdminListings(
    activeTab === 'listings' ? page : 1,
  );
  const { data: disputesData, isLoading: isLoadingDisputes } = useAdminDisputes(
    activeTab === 'disputes' ? page : 1,
  );
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
          <div className="white-card p-6 flex items-center gap-4">
            ...
            <div>
              ...
              {isLoadingStats ? (
                <div className="h-6 w-12 bg-neutral-200 animate-pulse mt-1 rounded" />
              ) : (
          ...
          <div className="white-card p-6 flex items-center gap-4">
            ...
            <div>
              ...
              {isLoadingStats ? (
                <div className="h-6 w-16 bg-neutral-200 animate-pulse mt-1 rounded" />
              ) : (
                    ...
                    usersData?.data.map((u) => (
                      <tr key={u.id} className="hover:bg-neutral-50">
                        ...
                        <td className="px-6 py-4">
                          <span
                            className={`px-2.5 py-1 rounded-pill text-xs font-medium ${u.role === 'ADMIN' ? 'bg-purple-100 text-purple-700' : 'bg-neutral-100 text-neutral-700'}`}
                          >
                            {u.role}
                          </span>
                        </td>
                  ...
                  {isLoadingListings ? (
                    ...
                    listingsData?.data.map((l) => (
                      <tr key={l.id} className="hover:bg-neutral-50">
                        ...
                        <td className="px-6 py-4">
                          <span
                            className={`px-2.5 py-1 rounded-pill text-xs font-medium ${l.isPublished ? 'bg-success-light text-success' : 'bg-amber-50 text-amber-700'}`}
                          >
                  ...
                  {isLoadingDisputes ? (
                    ...
                    disputesData?.data.map((d) => (
                      <tr key={d.id} className="hover:bg-neutral-50">
                        ...
                        <td className="px-6 py-4 text-primary-600 max-w-[150px] truncate">
                          {d.booking?.listing?.title}
                        </td>
                        <td className="px-6 py-4">{d.initiator?.name}</td>
                        <td className="px-6 py-4 max-w-[200px] truncate" title={d.reason}>
                          {d.reason}
                        </td>
                        <td className="px-6 py-4">
                          <span
                            className={`px-2.5 py-1 rounded-pill text-xs font-medium ${d.status === 'OPEN' ? 'bg-red-100 text-red-700' : d.status === 'RESOLVED' ? 'bg-success-light text-success' : 'bg-neutral-100 text-neutral-700'}`}
                          >
                            {d.status}
                          </span>
                        </td>
          ...
          <div className="p-4 border-t border-neutral-200 bg-neutral-50 flex justify-between items-center text-sm text-neutral-500">
            ...
            <div className="flex gap-2">
              <button
                ...
                disabled={page === 1}
                className="px-3 py-1 bg-white border border-neutral-200 rounded disabled:opacity-50 hover:bg-neutral-50"
              >
                Prev
              </button>
              <button
                onClick={() => setPage((p) => p + 1)}
                className="px-3 py-1 bg-white border border-neutral-200 rounded hover:bg-neutral-50"
              >
                Next
              </button>
            </div>
